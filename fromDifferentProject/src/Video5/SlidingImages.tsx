...
export const SlidingImages: React.FC<{images: string[], config: AnimationConfig, productName: string }> =
    ...
    return (
        <AbsoluteFill style={{ flexDirection: 'row' }}>
            <div style={{ position: "absolute", top: config.margins.top - 45, fontSize: 30, marginLeft: 50, color: "#000000",
                fontWeight: 800, fontFamily: "Antonio", textShadow: "rgba(0, 0, 0, 0.2) 0px 3px 10px", }}>
                {productName}
            </div>
            {imageItems.map(item => (
                <Img
